...
const COLORES_BASICOS = ["#fa8072", "#90ee90", "#f5deb3", "#dda0dd", "#4169e1"];
...
function dibujarRuleta(listaElementos) {
  ...
  if (totalElementos === 0) {
    ...
    contextoRuleta.arc(centro, centro, radio, 0, 2 * Math.PI);
    ...
    contextoRuleta.fillText("Sin elementos", centro, centro + 60);

    dibujarTrianguloRojo(tamanio, centro);
    return;
  }

  const anguloSector = (2 * Math.PI) / totalElementos;

  const tamanioLetra = Math.max(12, Math.min(34, 320 / totalElementos));

  elementosRuleta.forEach((elemento, indice) => {
    const anguloInicio = anguloActual + indice * anguloSector;

    const anguloFin = anguloInicio + anguloSector;

    contextoRuleta.beginPath();

    contextoRuleta.moveTo(centro, centro);

    contextoRuleta.arc(centro, centro, radio, anguloInicio, anguloFin);

    contextoRuleta.closePath();

    contextoRuleta.fillStyle = COLORES_BASICOS[indice % COLORES_BASICOS.length];
    ...
    contextoRuleta.translate(centro, centro);
    ...
    contextoRuleta.font = `${tamanioLetra}px Arial`;
    ...
    contextoRuleta.fillText(textoCorto, radio - 15, 0);
...
/* Triángulo rojo */
function dibujarTrianguloRojo(tamanio, centro) {
  contextoRuleta.beginPath();

  contextoRuleta.moveTo(tamanio - 38, centro);

  contextoRuleta.lineTo(tamanio - 4, centro - 16);
...
function obtenerIndiceSeleccionado() {
  ...
  const anguloSector = vueltaCompleta / elementosRuleta.length;

  const anguloBajoTriangulo =
    (((vueltaCompleta - (anguloActual % vueltaCompleta)) % vueltaCompleta) +
      vueltaCompleta) %
    vueltaCompleta;

  return (
    Math.floor(anguloBajoTriangulo / anguloSector) % elementosRuleta.length
  );
}
...
function girarRuleta() {
  ...
  cajaRespuesta.textContent = "Girando...";

  const anguloInicial = anguloActual;

  const vueltasAleatorias = 5 + Math.random() * 5;

  const anguloFinal = anguloInicial + vueltasAleatorias * 2 * Math.PI;
  ...
  function animarGiro(tiempoActual) {
    const progreso = Math.min((tiempoActual - tiempoInicio) / duracionGiro, 1);

    const progresoSuavizado = 1 - Math.pow(1 - progreso, 3);

    anguloActual =
      anguloInicial + (anguloFinal - anguloInicial) * progresoSuavizado;
...
function mostrarElementoSeleccionado() {
  ultimoSorteado = elementosRuleta[obtenerIndiceSeleccionado()];

  cajaRespuesta.textContent = ultimoSorteado.texto;
}
